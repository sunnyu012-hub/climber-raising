...
export const PROBLEMS: ClimbingProblem[] = [
  ...
  {
    id: 'cs-001',
    gymId: 'busan-cliffside',
    name: '혼자 깐 문제',
    grade: 3,
    wall: '오버행',
    desc: '세터 한 명이 자기 취향대로만 깐 문제. 친절함이 없다.',
    flavor: '벽에 매직으로 쓴 메모: "안 되면 다시 오세요."',
    recommend: ['power', 'mental'],
    reward: { exp: 130, money: 0, statExp: { power: 45, mental: 30 } },
    firstClearBonus: { exp: 90, money: 20000 },
    steps: [
      {
        id: 's1',
        situation: '스타트부터 두 손이 다 작은 크림프다.',
        line: '시작이 이러면 뒤는 어떻다는 거예요…',
        choices: [
          {
            id: 'a', label: '손가락을 세워 확실히 건다',
            moves: ['crimp'], stats: { power: 0.6, technique: 0.4 },
            baseChance: 0.6, cost: { hp: 7, fatigue: 8, finger: 6 }, luckMult: 1, reach: 'neutral',
            critText: '손끝이 딱 걸렸다. 이 정도면 붙을 만하다!',
            successText: '크림프를 잡고 몸을 띄웠다.',
            partialText: '손가락이 반쯤 풀렸다. 다시 잡았다.',
            fallText: '손가락이 버티지 못하고 미끄러졌다.',
          },
          {
            id: 'b', label: '몸을 벽에 붙여 부담을 나눈다',
            moves: ['flagging'], stats: { technique: 0.6, flexibility: 0.4 },
            baseChance: 0.56, cost: { hp: 6, fatigue: 7, knee: 3 }, luckMult: 1.1, reach: 'compressed',
            critText: '몸이 벽에 착 붙어 손 부담이 확 줄었다!',
            successText: '중심을 벽 쪽으로 옮겨 버텼다.',
            partialText: '몸이 살짝 열렸다. 팔에 힘이 들어갔다.',
            fallText: '몸이 벽에서 열리며 그대로 떨어졌다.',
          },
        ],
      },
      {
        id: 's2',
        situation: '다음 홀드가 멀다. 중간에 아무것도 없어 보인다.',
        line: '이건 리치 문제 아닌가요?',
        choices: [
          {
            id: 'a', label: '한 번에 튀어 잡는다',
            moves: ['dyno'], stats: { power: 0.7, mental: 0.3 },
            baseChance: 0.5, cost: { hp: 12, fatigue: 12, shoulder: 6 }, luckMult: 1.8, reach: 'far',
            critText: '허공에서 손이 정확히 홀드에 꽂혔다!',
            successText: '튀어서 잡았다. 팔이 얼얼하다.',
            partialText: '손끝이 스쳤다. 겨우 원래 자리로 돌아왔다.',
            fallText: '완전히 헛손질. 매트가 반겨준다.',
          },
          {
            id: 'b', label: '숨은 중간 홀드를 찾아 쓴다',
            moves: ['intermediate'], stats: { routefinding: 0.6, technique: 0.4 },
            baseChance: 0.55, cost: { hp: 8, fatigue: 9, finger: 4 }, luckMult: 1.2, reach: 'compressed',
            critText: '아무도 안 쓰던 중간 홀드를 찾았다. 이 문제 다시 봤다!',
            successText: '중간을 한 번 거쳐 안전하게 넘어갔다.',
            partialText: '중간 홀드가 생각보다 나쁘다. 손이 흔들린다.',
            fallText: '중간 홀드가 아예 안 잡혔다.',
          },
        ],
      },
      {
        id: 's3',
        situation: '마지막은 립 넘기. 발이 다 빠진 상태다.',
        line: '여기까지 왔으니까 끝은 봐야죠.',
        choices: [
          {
            id: 'a', label: '힐을 걸고 몸을 감아올린다',
            moves: ['heelhook'], stats: { flexibility: 0.5, power: 0.5 },
            baseChance: 0.55, cost: { hp: 11, fatigue: 12, knee: 7 }, luckMult: 1, reach: 'compressed',
            critText: '힐 하나로 몸이 통째로 올라갔다. 완등!',
            successText: '힐을 걸고 립을 넘었다. 완등이다!',
            partialText: '힐이 밀렸다. 다시 매달렸다.',
            fallText: '힐이 빠지면서 뒤로 넘어갔다.',
          },
          {
            id: 'b', label: '팔로 당겨 그냥 올라선다',
            moves: ['lockoff'], stats: { power: 0.8, mental: 0.2 },
            baseChance: 0.48, cost: { hp: 15, fatigue: 15, shoulder: 9 }, luckMult: 1.2, reach: 'neutral',
            critText: '팔만으로 립을 넘었다. 오늘 팔에게 고맙다!',
            successText: '억지로 당겨 올라섰다. 완등!',
            partialText: '팔이 안 접힌다. 한 번 더 힘을 모은다.',
            fallText: '립 앞에서 팔이 다 풀렸다.',
          },
        ],
      },
  ...
  // =============================== 7 (클리프사이드 프로젝트)
  {
    ...
    grade: 4,
    wall: '루프',
    desc: '천장에 붙은 문제. 완등자 칸이 비어 있다.',
    flavor: '완등자 이름표 자리에 아직 아무것도 없다.',
    recommend: ['power', 'flexibility'],
    reward: { exp: 220, money: 0, statExp: { power: 55, flexibility: 40, mental: 35 } },
    firstClearBonus: { exp: 180, money: 50000 },
    achievement: { id: 'ach-cliffside-project', name: '첫 완등자', desc: '아직 아무도 못 푼 것을 완등했다' },
    steps: [
      {
        id: 's1',
        situation: '천장에 매달리는 것부터가 일이다.',
        line: '어… 발이 어디로 가야 하죠?',
        choices: [
          {
            id: 'a', label: '토훅으로 발을 먼저 건다',
            moves: ['toehook'], stats: { flexibility: 0.6, technique: 0.4 },
            baseChance: 0.52, cost: { hp: 10, fatigue: 11, knee: 6 }, luckMult: 1, reach: 'compressed',
            critText: '발이 천장에 딱 걸렸다. 몸이 안 흔들린다!',
            successText: '토훅으로 몸을 고정했다.',
            partialText: '발이 미끄러지며 몸이 크게 흔들렸다.',
            fallText: '발이 빠지고 몸이 통째로 흔들려 떨어졌다.',
          },
          {
            id: 'b', label: '팔로 버티며 그냥 매달린다',
            moves: ['lockoff'], stats: { power: 0.8, mental: 0.2 },
            baseChance: 0.46, cost: { hp: 14, fatigue: 15, shoulder: 8 }, luckMult: 1.2, reach: 'neutral',
            critText: '팔만으로 천장에 매달렸다. 코어가 살아있다!',
            successText: '팔로 버티며 자세를 잡았다.',
            partialText: '몸이 아래로 처졌다. 겨우 버틴다.',
            fallText: '팔이 풀리며 매트로 떨어졌다.',
          },
        ],
      },
      {
        id: 's2',
        situation: '천장 한가운데. 손을 바꿔야 다음이 닿는다.',
        line: '여기서 쉬는 건 불가능하겠네요.',
        choices: [
          {
            id: 'a', label: '손을 빠르게 맞바꾼다',
            moves: ['matching'], stats: { technique: 0.6, power: 0.4 },
            baseChance: 0.5, cost: { hp: 12, fatigue: 13, finger: 7 }, luckMult: 1.1, reach: 'compressed',
            critText: '손이 딱 맞바뀌었다. 리듬이 살았다!',
            successText: '손을 바꿔 자세를 다시 잡았다.',
            partialText: '손이 겹치며 한 손이 밀렸다.',
            fallText: '손을 바꾸다 둘 다 놓쳤다.',
          },
          {
            id: 'b', label: '한 손으로 멀리 뻗는다',
            moves: ['dyno'], stats: { power: 0.7, mental: 0.3 },
            baseChance: 0.42, cost: { hp: 16, fatigue: 16, shoulder: 9 }, luckMult: 1.9, reach: 'far',
            critText: '천장에서 다이노를 성공시켰다. 사람들이 소리를 질렀다!',
            successText: '멀리 뻗어 잡았다. 심장이 뛴다.',
            partialText: '손끝만 스쳤다. 겨우 돌아왔다.',
            fallText: '천장에서 그대로 떨어졌다. 등부터 착지.',
          },
        ],
      },
      {
        id: 's3',
        situation: '립으로 나가는 마지막 한 동작. 팔에 남은 게 없다.',
        line: '남은 힘, 지금 다 쓸게요.',
        choices: [
          {
            id: 'a', label: '몸을 감아 립으로 나간다',
            moves: ['heelhook'], stats: { flexibility: 0.5, power: 0.3, mental: 0.2 },
            baseChance: 0.45, cost: { hp: 16, fatigue: 17, knee: 9 }, luckMult: 1.1, reach: 'compressed',
            critText: '천장에서 립으로 몸이 통째로 넘어갔다. 완등!',
            successText: '힐을 걸고 립으로 넘어갔다. 완등이다!',
            partialText: '립 앞에서 몸이 멈췄다. 아직 안 떨어졌다.',
            fallText: '립을 못 넘고 그대로 떨어졌다.',
          },
          {
            id: 'b', label: '남은 힘으로 당겨 올린다',
            moves: ['lockoff'], stats: { power: 0.9, mental: 0.1 },
            baseChance: 0.4, cost: { hp: 20, fatigue: 20, shoulder: 11 }, luckMult: 1.3, reach: 'neutral',
            critText: '마지막 한 번을 당겨냈다. 완등자 칸에 이름이 들어간다!',
            successText: '억지로 당겨 립을 넘었다. 완등!',
            partialText: '팔이 반쯤 접히다 멈췄다.',
            fallText: '팔이 완전히 풀렸다. 오늘은 여기까지.',
          },
        ],
      },
    ],
